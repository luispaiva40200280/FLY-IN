...
import heapq
from ..models.network import Drone, Map
...
class Navigator:
    ...
    def _register_path(self, drone: Drone, path: list[str]) -> None:
        ...
        for turn in range(len(path)):
            curr_node = path[turn]
            if not (turn + 1 < len(path) and curr_node == path[turn + 1]):
                nbr_drones_turn = (self.zone_reservations.get(
                    (curr_node, turn), 0) + 1)
    ...
    def _get_ideal_cost(self, current_zone: str, end_zone: str) -> int:
        ...
        """
        priority_queue: list[tuple[int, str]] = [(0, current_zone)]
        visited: set[str] = set()
